...
export default class MRBrginnerBlock extends MetadataNode {
    ...
    protected initFromState(tick: Tick): void {
        ...
        this.unvisitedNodes =
            tick.blackboard.get("unvisitedNodes", tick.tree.id, this.id) || RandomizationHelper.shuffle(_.range(this.children.length));
    ...
    tick(tick: Tick): ITickResult {
        ...
        if (this.unvisitedNodes.length === 0) {
            ...
            // BegginerBlock failed
            else if (this.checkIfBeginnerBlockFailed()) {
    ...
    private checkIfBeginnerBlockFailed(): boolean {
        let beginnerBlockFailed = true;
        this.failedNodes.forEach((child) => {
            if (this.childrenMetadata[child].failCount != 0) {
                beginnerBlockFailed = false;
            }
        });
        return beginnerBlockFailed;
    }
